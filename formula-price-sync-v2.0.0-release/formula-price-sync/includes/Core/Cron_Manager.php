...
class Cron_Manager {
	...
	const RATE_LIMIT_SECS  = 300; // 5 minutes between cron runs.
	...
	public static function handle_cron_request() {
		...
		// Optionally lock so concurrent requests are rejected.
		set_transient( 'fps_cron_running', true, MINUTE_IN_SECONDS );

		$result = self::run_sync();

		// Unlock.
		delete_transient( 'fps_cron_running' );
	...
	public static function token_field_cb() {
		...
		<input type="text" name="<?php echo esc_attr( self::OPTION_KEY ); ?>"
			...
			class="regular-text"
			placeholder="<?php esc_attr_e( 'یک توکن تصادفی وارد کنید (مثلاً aB3xK9mQ2)', 'formula-price-sync' ); ?>"
			autocomplete="off">
		<p class="fps-desc">
			<?php esc_html_e( 'از این توکن در URL کرون استفاده می‌شود. آن را امن نگه دارید.', 'formula-price-sync' ); ?>
		</p>
		<?php if ( ! empty( $token ) ) : ?>
			<p class="fps-desc" style="color:var(--fps-color-success);">
				...
				printf(
					...
					esc_html__( 'URL فعال: %s', 'formula-price-sync' ),
